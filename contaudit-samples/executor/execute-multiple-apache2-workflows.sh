...
max=$1
echo $(date +"%Y-%m-%d %T,%N") '[execute-multiple-apache2-workflow] Starting ' $max ' workflows asynchronously...'
for thread in $(seq 1 $max);
do
    executeWrapper "$thread" "./contaudit-samples/executor/artifacts/apache2.workflow $thread" &
done
wait
echo $(date +"%Y-%m-%d %T,%N") '[execute-multiple-apache2-workflow] Finished...'