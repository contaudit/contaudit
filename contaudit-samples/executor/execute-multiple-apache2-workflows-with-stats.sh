...
echo $(date +"%Y-%m-%d %T,%N") '[execute-multiple-apache2-workflow-with-stats] Starting statistics...'
sudo docker stats -a --format "{{ json . }}" > "experiment_${max}_stats.jsonl" &
stats=$!

echo $(date +"%Y-%m-%d %T,%N") '[execute-multiple-apache2-workflow-with-stats] Starting '$max' workflows asynchronously...'
pids=""
...
sudo kill -9 $stats
echo $(date +"%Y-%m-%d %T,%N") '[execute-multiple-apache2-workflow-with-stats] Finished...'